...
declare namespace FMOD {
  ...
  export interface System {
    ...
    getChannelsPlaying(
      channels: Out<number>,
      realchannels: Out<number>
    ): RESULT;

    getDSPBufferSize(
      bufferlength: Out<number>,
      numbuffers: Out<number>
    ): RESULT;
    /**
    ...
    getDSPInfoByPlugin(
      handle: number,
      description: Out<DSP_DESCRIPTION>
    ): RESULT;
...
Also defines the number of channels in the unit that a read callback will process, and the output signal of the unit.
  ...
  export interface EventDescription {
    ...
    getParameterDescriptionByName(
      name: string,
      parameter: STUDIO_PARAMETER_DESCRIPTION
    ): RESULT;
    /**
  ...
  export interface CommandReplay {
    ...
    getCurrentCommand(
      commandindexOut: Out<number>,
      currenttimeOut: Out<number>
    ): RESULT;
    ...
    setCreateInstanceCallback(
      callback: FMOD.STUDIO_COMMANDREPLAY_CREATE_INSTANCE_CALLBACK
    ): RESULT;

    setFrameCallback(
      callback: FMOD.STUDIO_COMMANDREPLAY_FRAME_CALLBACK
    ): RESULT;

    setLoadBankCallback(
      callback: FMOD.STUDIO_COMMANDREPLAY_LOAD_BANK_CALLBACK
    ): RESULT;
